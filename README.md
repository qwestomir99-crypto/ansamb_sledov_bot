...
### Что работает
- Бот (`bot.py`) — Flask, keep-alive, маршрут `/new`
- Агент (`agent.py`) — Yandex GPT + fallback, keep-alive
- Админка в Telegram — кнопки, цитаты, посты, диагностика
- Цитаты, публикации, VK-постинг (текст/фото)

### Что ждёт 1-го числа
- Сброс кеша Render → веб-морда `/new` заработает
- Сброс лимитов сборки → агент (Yandex GPT) оживёт
- Новый токен VK → видео через бота

### В разработке / отложено
- Адаптивные режимы (выключены, включим позже)
- Шаббат (проверим в субботу)
- Переименование модулей (финальный ритуал)

### Важно
- Keep-alive пинг уже встроен в бота и агента
- Агент сейчас отвечает локальной заглушкой
- Главный README — этот файл (`README.md`)
