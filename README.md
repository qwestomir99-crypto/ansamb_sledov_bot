...
| **Дебаггер** | `debug_utils.py` | Единое логирование, ротация, отчёты в Telegram и веб-морду. |
| **Веб-морда** | `services/app.py` | Единый интерфейс: сообщения (VK+TG), YouTube-прокси, дебаггер. |
...
| `WEB_THEME` | Тема веб-морды (macos.css / dark.css). |
...
- Логи (`debug.log`) ротируются (1 МБ, хранится только последний бэкап).
...
### Что работает
- Бот (`bot.py`) — команды, админка, цитаты, публикации, `/debug`
...
- Дебаггер — единое логирование, ротация, отчёты в Telegram и веб-морду
- Веб-морда — лента сообщений (VK+Telegram), YouTube-прокси, дебаггер
