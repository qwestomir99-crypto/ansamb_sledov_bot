...
def refresh_vk_token():
    ...
    try:
        ...
        if "access_token" in data:
            ...
            if "refresh_token" in data: os.environ["VK_REFRESH_TOKEN"] = data["refresh_token"]
...
def vk_callback():
    ...
    try:
        r = req.post("https://id.vk.com/oauth2/auth", data=params, timeout=10)
        data = r.json()
        if "access_token" in data: return f"<h2>✅ Токен получен!</h2><p><b>Device ID:</b> {device_id}</p><p><b>Access Token:</b><br><textarea rows='3' style='width:100%'>{data['access_token']}</textarea></p><p><b>Refresh Token:</b><br><textarea rows='3' style='width:100%'>{data.get('refresh_token', '')}</textarea></p>"
        else: return f"<h2>❌ Ошибка: {data.get('error')}</h2>", 500
    except Exception as e: return f"<h2>❌ Ошибка: {e}</h2>", 500
